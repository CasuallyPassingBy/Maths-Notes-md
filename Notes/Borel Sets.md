...
We define the *Borel sets* as those sets of any topological space $(X, \tau)$ that can be obtained from open and closed sets by means of repeatedly taking countable unions and intersections. 

**Def:** A set $B \subseteq X$ is *Borel* if it belongs to every system of sets $\mathcal S\subseteq \mathcal P(X)$ with the following properties:
- All open sets and closed sets belong to $\mathcal S$
- If $B_n \in S$ for each $n < \omega$, then $\bigcup_{n  <\omega} B_n$ and  $\bigcap_{n  <\omega} B_n$ belong to $\mathcal S$
$\mathcal B$ denotes the system of all Borel sets

In other words, $\mathcal B = \bigcap \{\mathcal S \subseteq \mathcal P(X)\mid \mathcal S \text{ with both properties}\}$ is the smallest collection of sets of reals that contains all open and all closed sets and is "closed" under countable unions and intersections. 
...
{\mathbf \Sigma}_1^0&:= \{B \subseteq X \mid B \text{ is open}\} \\
{\mathbf \Pi}_1^0&:= \{B \subseteq X \mid B \text{ is closed}\} \\
\end{align*} 
$$
...
**Lemma:** 
- For every $\alpha <\omega_1$, $\mathbf \Sigma_\alpha^0 \subseteq \mathbf \Pi_{\alpha+1}^0$ and $\mathbf \Pi_\alpha^0 \subseteq \mathbf \Sigma_{\alpha+1}^0$
...
Every uncountable real Borel set contains a [[Perfect and Connected Sets in R|perfect subset]]

Let $f: X \to X$ be a continuous function. If $B \in \mathbf \Sigma_\alpha^0$, then $f^{-1}[B] \in \mathbf \Sigma_\alpha ^0$. Similarly for $\mathbf \Pi_\alpha^0$. 

The cardinality of standard real Borel algebra is $2^{\aleph_0}$.
