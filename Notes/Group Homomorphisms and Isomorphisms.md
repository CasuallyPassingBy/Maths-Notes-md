...
**Def:** When a homomorphism $\phi: G \to G'$ is bijective, then we say that $\phi$ is a *group isomorphism*. We also say that $G$ and $G'$ are *isomorphic* if there's an isomorphism between them. It is denoted as $(G, \cdot, e) \cong (G', \cdot', e')$ or $G \cong G'$ when the operation an identity are clear. In the case that $G' = G$, then $\phi$ is called as an *automorphism*. 
...
**Obs:** Let $G$ be a cyclic group, $H$ be a group, and $f: G \to H$. The image of $f$ is the cyclic group generated by $f(a)$ where $a$ is a generator of $G$. 

**Prop:** Let $G$ be a cyclic group and $f: G \to H$ be any homomorphism. There is a unique $n \in \Bbb Z$ such that $f(g) = g^n$ for all $g\in G$. Additionally, if $G$ is infinite, then $n$ is uniquely determined by $f$. In this case, we call $n$ the degree of $f$, denoted as $\deg f := n$.  ^2b7d8f

**Def:** Let $G$ and $H$ be groups. Then, we can define the set of group homomorphism from $G$ to $H$ is denoted as $\text{Hom}(G, H)$. 

**Def:** For any fixed abelian group $G$, there is a contravariant functor from the category of abelian groups to itself that sends each group $X$ to the group $\text{Hom}(X, G)$ of homomorphisms into $G$, and each homomorphism $f: X \to Y$ to the induced homomorphism $f^*:H(Y, G) \to\text{Hom}(X, G)$ given by  $$f^*(\varphi) = \varphi \circ f.$$
This basically gives us a nice generalisation to [[Dual Vector Spaces|dual vector spaces and dual operators]]