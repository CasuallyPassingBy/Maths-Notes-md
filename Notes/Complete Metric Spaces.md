...
Let $D$ be a dense subset of $X$, $Y$ be a complete metric space, and $f:D\to Y$ be uniformly continuous. Then there’s a unique function $f^*:X\to Y$ that is uniformly continuous and ${f^*|_D = f}$. 

This is similar when we extended a function from $A$ to $\overline A$ in $\Bbb R^n$
