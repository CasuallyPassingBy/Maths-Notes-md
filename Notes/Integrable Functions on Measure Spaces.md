...
Links: [[Measure Spaces and Measurable Spaces]], [[Measurable Functions]], [[Integration of Simple Functions]]
...
**Prop:** If an integrable function $f$ is non negative a.e., then $$\int f\, d\mu \ge 0$$
**Cor:** If $f$ and $g$ are integrable functions such that $f\ge g$ a.e., then $$\int f\, d\mu \ge \int g \, d\mu.$$
**Cor:** If $f$ and $g$ are integrable functions, then $$\int |f+g| \, d\mu \ge \int|f| \, d\mu + \int|g| \, d\mu.$$
**Cor:** If $f$ is an integrable function, then $$\left|\int f\, d\mu\right| \le \int|f|\, d\mu.$$
**Cor:** If $f$ is an integrable function, $\alpha, \beta \in \Bbb R$ and $E$ is measurable set such that for $x\in E$, $\alpha \le f(x) \le \beta$, then $$\alpha \mu(E) \le \int_E f\, d\mu \le \beta\mu(E).$$
...
**Prop:** If $f$ is an integrable function and $E$ is a set of measure zero, then $$\int_E f\, d\mu  =0.$$
**Prop:** If $f$ is an integrable function which is positive a.e. on a measurable set $E$, and if $\int_E f\,d\mu = 0$, then $\mu(E) = 0$.

**Prop:** If $f$ is an integrable function such that $\int_F f\, d\mu =0$ for every measurable set $F$, then $f = 0$ a.e.
...
**Obs:** If $X = \Bbb N$ with $\mathcal S = P(\Bbb N)$, and $\mu(E) = |E|$. Then $f: X \to \Bbb R$ is integrable iff $\sum_{n <\omega} |f(n)|$ converges. 
# Sequences
...
**Th:** If $\{f_n \mid n <\omega\}$ is a mean fundamental sequence of integrable functions, then there exists an integrable function $f$ such that $\|f_n-f\|_1 \to 0$, and $$\int f_n \, d\mu \to \int f\, d\mu$$ as $n\to \infty$. 
...
**Lebesgue's Bounded Convergence Theorem:** If $\{f_n\mid n <\omega\}$ is a sequence of integrable functions which converges in measure to $f$ (or else converges to $f$ a.e.), and if $g$ is integrable function such that $|f(x)| \le |g(x)|$ a.e., for every $n<\omega$, then $f$ is integrable and the sequence $\{f_n \mid n <\omega\}$ converges to $f$ in the mean.
