...
- **Double of a Smooth Manifold With Boundary:** Let $M$ be a smooth manifold with boundary. The *double of $M$* is the manifold $D(M) := M \cup_\text{id} M$; it is obtained from $M \sqcup M$ by identifying each boundary point in one copy of $M$ with the same boundary in the other. It is a smooth manifold without boundary, and contains two regular contains diffeomorphic to $M$. It is easy to check that $D(M)$ is compact iff $M$ is compact, and connected iff $M$ is connected. Sometimes it is useful to extend the definition to manifolds without boundary by defining $D(M) = M \sqcup M$ when $\partial M = \varnothing.$ ^fe4a85

**Prop:** Suppose $M_1$ and $M_2$ are connected smooth $n$-manifolds and $M_1\# M_2$ is their connected sum. We can choose the smooth structure on $M_1\# M_2$ in such a way that there are open subsets $\widetilde M_1, \widetilde M_2 \subseteq M_1\# M_2$, that are diffeomorphic to $M_1\setminus \{p_1\}$ and $M_2\setminus\{p_2\}$, respectively, such that $\widetilde M_1 \cup \widetilde M_2 = M_1 \# M_2$ and $\widetilde M_1 \cap \widetilde M_2$ is diffeomorphic to $(-1, 1) \times \Bbb S^{n-1}$. 
