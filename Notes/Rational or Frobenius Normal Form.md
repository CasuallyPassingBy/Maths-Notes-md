...
For this section $C_x$ refers to the $T$-cyclic subspace generated by $x$, and the ordered set
...
Furthermore, the characteristic polynomial of $A$ given by
$$ \chi_A(t) = (-1)^k \left(\sum_{i = 0}^{k-1}a_i t^i +t^k\right) =0  $$

The matrix $A$ is called the *companion matrix* of the monic polynomial $h (t)= \sum_{i = 0}^{k-1}a_i t^i +t^k$. Every monic polynomial has a companion matrix, and the characteristic of the companion matrix of a monic polynomial $g(t)$ of degree $k$ is equal to $(-1)^k g(t)$.

The monic polynomial $h(t)$ is also the minimal polynomial of $A$. Since $A$ is the matrix representation of the restriction to $C_x$, $h(t)$ is also a minimal polynomial of the restriction, and $h(t)$ is a $T$-annihilator of $x$.
...
**************Th:************** Let $v_1, v_2, \dots, v_k$ be distinct vectors in $K(\phi, T)$ such that$$ S_1 = \bigcup_{i = 1}^k \beta_{v_i} $$
...
- For some $w_1, w_2, \dots, w_s \in N(\phi(T))$, $\beta$ can be extended to the linearly independent set$$ \beta' =\beta \cup \bigcup_{i =1}^s \beta_{w_i} $$whose span contains $N(\phi(T))$
