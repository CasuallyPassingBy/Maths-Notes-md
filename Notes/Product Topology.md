...
**Def:** **Def:** We say that a topological property $P$ is *multiplicative* if for any family $\{X_s\mid s\in S\}$ of spaces with property $P$, the sum $\prod_{s\in S}X_s$ also has the property $P$. Similarly, we say that a property is $\kappa$-*productive* if we add the condition that $|S|<\kappa$, and *finitely productive* if $|S| <\aleph_0.$ 
