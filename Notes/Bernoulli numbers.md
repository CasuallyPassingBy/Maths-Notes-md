...
We have that Ross Tang have an explicit formula for the Bernoulli numbers:$$ B_{2n} = \frac{2n}{2^{2n}-4^{2n}}\sum_{k = 1}^{2n}\sum_{j = 0}^k \frac{(-1)^j (k-2j)^{2n}}{2^ki^kk}$$the explicit formula that appears on Wikipedia is $$B_m = \sum_{k=0}^m \frac1{k+1} \sum_{j=0}^k \binom{k}{j} (-1)^j j^m$$
If we recall the identity $$2\coth(2z)-\coth z = \tanh z$$Then we can get the identity $$\tanh z = \sum_{n = 1}^\infty \frac{4^n(4^n -1)B_
2n}{(2n)!}z^{2n-1}$$if we add that $\tan z = -i \tanh(iz)$, then $$\tan z = \sum_{n = 1}^\infty (-1)^{n-1}\frac{4^n(4^n -1)B_{2n}} {(2n)!}z^{2n-1} $$