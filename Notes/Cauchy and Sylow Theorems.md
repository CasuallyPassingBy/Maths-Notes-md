...
**Def:** If $G$ is a finite group of order $n$ and $p$ is a prime such that $p^m \;\|\;n$ with $m \ge 1$, a subgroup $L \le G$ is with order $p^m$ is called a *Sylow $p$-subgroup of $G$*. 
...
**Sylow's Thir Theorem:** Let $G$ be a finite group of order $n$, $p$ a prime such that $p \mid n$, and $n_p$ is the Sylow $p$-subgroups of $G$, then $$n_p \equiv 1 \pmod p, \quad n_p = [G: N_G(P)],$$where $P$ is any Sylow $p$-subgroup of $G$, and $n_p \mid n$.
