...
**Def:** Let $X$ be a $G$-set, then we the subset of $X^G$ of $X$ is called the *fixed-point set* or the *set of $G$-fixed points*, of the $G$-action on $X$. We can also call them the *invariant elements,* and we can denote it as $X^ G$ or $\text{Fix}_G(X)$.
