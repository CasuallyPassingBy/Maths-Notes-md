...
**Cor:** Let $G$ be a Lie group, let $\frak g$ be its Lie algebra, then the Lie group homomorphism $\psi: G \to \Bbb R$ defined by $\psi(g) := \det(\text{Ad}(g))$ has an induced Lie algebra homomorphism $$\psi_*(X) = \text{tr}(\text{ad}_X).$$
