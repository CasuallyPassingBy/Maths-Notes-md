...
If $(M, g)$ is a Riemannian manifold, then we can also define an inner product on $T_p^*M$. We just need a symmetric  $2$-contravariant tensor field. In any smooth local coordinates $(x^i)$, our symmetric tensor can be written as $$b = b^{ij} \frac{\partial}{\partial x^i} \otimes \frac{\partial}{\partial x^j},$$but we can make use of the symmetric product to get $$b = b^{ij} \frac{\partial}{\partial x^i} \otimes \frac{\partial}{\partial x^j} = b^{ij} \frac{\partial}{\partial x^i}  \frac{\partial}{\partial x^j}. $$If we want it to be consistent with the musical isomorphisms, given two vectors $v, w\in T_p M$, we would like have an equality $$\langle v, w\rangle_g = \langle v^\flat, w^\flat\rangle_b.$$If we expand this we get that  $$g_{ij}v^i w^j = b^{k\ell}g_{ik}g_{j\ell} v^i w^j, $$when we simplify this we get $$\delta^k_j = b^{k\ell}g_{j\ell}.$$This exactly, means that $(b^{ik})$ is the inverse of the matrix $(g_{ij})$. This means that $g^{ij} = \langle dx^i, dx^j\rangle_g$. 
