...
If $G = D_n$, and $K = R_n$ (the rotation subgroup), then $R_n \trianglelefteq D_n$, and $[D_n: R_n] = 2$. Note that $\Bbb Z/2\Bbb Z$ is isomorphic to the subgroup $H$ generated by any reflection. We have that $$ D_n = R_n \rtimes C_2.$$Note that we are using the [[semidirect product of groups]] to get how reflections interact with the rotations 