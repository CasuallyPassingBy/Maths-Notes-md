...
Links: [[Tensor Product of Modules]], [[Tensor Product of Modules]], [[Graded Ring]], [[Module and Algebra (Structure)]], [[Bases and Dimension]]
...
**Prop:** If $M$ is any $R$-module over the commutative ring $R$ then $T(M)$ is an $R$-algebra containing $M$ with multiplication defined by mapping $$(m_1 \otimes \dots \otimes m_i) (m_1' \otimes \dots \otimes m_j') = m_1 \otimes \dots \otimes m_i\otimes m'_1 \otimes \dots \otimes m_j'$$and extended to sums via the distributive laws. With respect to muplitlications $M^{\otimes i} M^{\otimes j} \subseteq M^{\otimes (i+j)}$ 
...
**Prop:** Let $V$ be a finite dimensional vector space over the field $F$ with basis $\beta = \{v_1, \dots, v_n\}$. Then the $k$-tensors $$v_{i_1} \otimes v_{i_2} \otimes \otimes \dots \otimes v_{i_k} \qquad \text{with } v_{i_j} \in \beta$$are a vector space basis for $V^{\otimes k}$ over $F$. In particular, $\dim_F (V^{\otimes k}) = n^k$. 
