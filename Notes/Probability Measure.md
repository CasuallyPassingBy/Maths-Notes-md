...
********Def:******** Let $(\Omega, \mathscr F)$ is a [[Rings and Algebras of Sets|measurable space]]. A probability measure is a function $\Bbb P: \mathscr F\to [0,1]$, that satisfies
- $\Bbb P(\Omega)= 1$
- $\Bbb P(A) \ge0$ for any $A \in \mathscr F$
- If $A_1, A_2, \dots\in \mathscr F$ are mutually disjoint, $A_n \cap A_m = \varnothing$ for $n \ne m$, then$$ \Bbb P\left(\bigcup_{n = 1}^\infty A_n\right) = \sum_{n = 1}^\infty \Bbb P(A_n) $$
We say that the triple $(\Omega, \mathscr F, P)$ we call it a probability space. We can write this as $(\Omega, {\scr F}, P)$ is a measure space and $\Bbb P(X) = 1$. 

**Prop:** Let $(\Omega, \mathscr F, P)$ be a probability space, the
- $\Bbb P (\varnothing ) = 0$
- If $A_1, \dots, A_n \in \mathscr F$, are mutually disjoint, then$$ \Bbb P\left(\bigcup_{k = 1}^n A_k\right)=\sum_{k = 1}^n \Bbb P(A_k) $$
