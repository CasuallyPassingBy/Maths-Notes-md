...
**Def:** Let $f:D\subseteq\Bbb R^n\to \Bbb R^m$ be differentiable $x \in D$. Then the **unique** close linear approximation of $\delta f_x$ at $\mathbf 0$ is denoted as $df_x$ and it is called the differential of $f$ at $x$.

**Def:** We call the rank of $f$ at $x$ as the rank of $df_x$. Meaning, $\text{rank} f(x) := \text{rank} (df_x)$  
...
- Properties
    ...
    $f+g$ is differentiable at $x_0$$$ d(f+g)_{x_0} = df_{x_0} +dg_{x_0} \quad \text{ or } \quad D(f+g)(x_0) = Df(x_0) +Dg(x_0)$$
    $\lambda f$ is differentiable at $x_0$	    $$ d(\lambda f)_{x_0} = \lambda df_{x_0} \quad \text{ or }\quad D(\lambda f)(x_0)= \lambda Df(x_0)$$
...
********Th:******** In the special case of considering ${\hat u}\in \mathbb S^{n-1}$ and $df_x({\hat u})$ then it is obtained that: 
...
**Def:** A function $f:D\subseteq\Bbb R^n\to \Bbb R^m$ is **smooth** if $f$ is a $\cal C^1$ function and if for all $x \in D$, the Jacobian $J_f(x)$ is of the maximum possible rank $\min(m,n)$. A function $g:S\subseteq\Bbb R^n\to \Bbb R^m$ whose domain is not an open subset of $\Bbb R^n$ is said to be **smooth** if it can be extended to a smooth function on an open domain containing $S$.
