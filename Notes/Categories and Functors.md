...
- Two class functions
	...
	Which assign to each morphism $f \in \text{mor}(\mathsf C)$ a unique pair of objects $A, B\in \text{ob}(\mathsf C)$ such that $\text{dom}(f)= A$ and $\text{cod}(f) = B$. We write the $f:A \to B$ to indicate that $f$ has domain $A$ and codomain $B$. We use the notation $\text{Mor}_{\mathsf C}(A,B)$ to denote the class of al morphisms from $A$ to $B$.
...
We say that a category is *small* if $\text{ob}(\mathsf C)$ is a set. Additionally, we say that $\mathsf C$ is *locally small* if for any two objects $A$ and $B$ the class $\text{mor}_{\mathsf C}(A, B)$ is a set. 
