...
Links: [[Symplectic Forms on Smooth Manifolds]], [[Tangent Distributions and Involutivity on Smooth Manifolds]]. [[The Lie Derivative]], [[Hamiltonian Vector Fields]], [[Integral Curves, Flows and Flowouts on Smooth Manifolds]]
...
**Prop:** Suppose $(M, H)$ is a contact manifold of dimension $2n+1$. 
- If $n$ is odd, then $M$ is orientable;
- if $M$ is orientable and $M$ is even iff there exists a global contact form for $H$.
...
**Contact Darboux Theorem:** Suppose $\theta$ is a contact form on a $(2n+1)$-dimensional manifold $M$. For each $p\in M$, there are smooth coordinates $(x^1,\dots, x^n, y^1,\dots, y^n, z)$ centred at $p$ in which $\theta$ has the form  $$\theta = dz - \sum_{i = 1}^n y^i dx^i.$$
...
**Lemma:** Let $(M, H)$ is a contact manifold and $\theta$ is a contact form for $H$. If $X$ is a contact vector field, iff  $$\mathcal L_X\theta = f\theta, $$for some smooth function $f\in \mathcal C^\infty(M)$.
...
**Prop:** Suppose $(M, H)$ is a contact manifold and $X$ is a smooth vector field on $M$. Then $X$ is a the Reeb field of some contact form for $H$ iff it is a contact field that takes no values in $H$.
