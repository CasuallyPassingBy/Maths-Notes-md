...
Links: [[Group Actions]], [[Continuous Actions of Groups]], [[Lie Groups]], [[Representations of Groups]], [[General Linear Group]], [[Lie Algebra]], [[Lie Algebra of a Lie Group]]
...
**Obs:** Any representation $\rho$ yields a smooth left action of $G$ on $V$, defined by $$g \cdot v := \rho(g) v, \qquad \text{for }g \in G, v \in  V$$**Def:** If $G$ is a Lie group, an action $G$ on a finite dimensional vector space $V$ is said to be *linear* if for each $g \in G$, the map from $V$ to itself given by $v \mapsto g\cdot v$ is linear:
