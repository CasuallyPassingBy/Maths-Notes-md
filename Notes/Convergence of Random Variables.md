...
Links: [[Random Variables]], [[Lp spaces]], [[Convergence of Measurable Functions]], [[Lebesgue Integral on Measure Spaces]]

Let $X_1, X_2, \dots, X_n, \dots$ be a sequence of random variables. 
...
We can denote it as $X_n \to X$, or if we want to specify, $X_n \stackrel{\Bbb P}{\longrightarrow}X$. 
## Almost Everywhere Convergence

The sequence of random variables $X_n$, converges *converges almost surely* to $X$ if $$\Bbb P(\{\omega \in \Omega \mid \lim_{n \to \infty} X_n(\omega) = X(\omega)\} = 1$$or $$\Bbb P\left(\lim_{n\to \infty} X_n = X\right) = 1$$We can denote is as $X_n \stackrel{a.s.} {\longrightarrow} X$, or $\lim\limits_{n \to \infty} X_n = X\; a.s.$ 
...
The sequence of random variables $X_n$ converges to $X$ in $L^p$ if $$\lim_{n \to \infty} E|X_n - X|^p = 0$$This type of convergence is also called $L^p$ *convergence* and it's denoted as $X_n \stackrel{L^p}{\longrightarrow}X.$

## Convergence in distribution or Weak Convergence



The sequence of random variables $X_n$ converges to $X$ in *distribution* if for all $x$ where the function $F_X$ is continuous. it satisfies that $$\lim_{n \to \infty} F_{X_n}(x) = F_X(x)$$Where $F_{X_n}$ is the cdf of $X_n$, and $F_X$ being the cdf $X$. We denote denote it as $X_n \stackrel{d}{\longrightarrow}X$, $X_n \stackrel{\mathcal D}{\longrightarrow} X$, or lastly $F_{X_n} \stackrel{d}{\longrightarrow}F_X$. This type of convergence is also known as *weak convergence* since it less restrictive than the others. 
...
### Fatou's Lemma
Let $X_1, X_2, \dots$ be a sequence of non negative random variables, then $$\Bbb E\left[\liminf_{n \to \infty} X_n\right] \le \liminf_{n\to\infty} \Bbb E[X_n].  $$