...
**Def:** A $2$-tensor $\omega$ on a finite dimensional real vector space $V$ is said to be *nondegenrate* if $\omega(u, v) = 0$ for all $Y\in V$ implies $X = 0$. This in particular remainds
...
- The linear map $\widetilde\omega: V \to V'$ defined by $\widetilde\omega(u)(v) := \omega(u, v)$ is invertible. 
