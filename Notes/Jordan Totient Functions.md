...
in the terms of Dirichlet convolution we can simplify it as $$ J_k * 1 = \operatorname{Id}_k $$and using the Möbius Inversion formula we get $$ \sum_{d \mid n} \mu(d) \left(\frac{n}{d}\right)^k = J_k(n) $$or $$ \mu * \operatorname{Id}_k = J_k $$**Th:** for any $r$ and $s$ and $n >1$, then$$ \sum_{d\mid n} d^s J_r(d) J_s\left(\frac{n}{d}\right) = J_{r+s}(n) $$or $$ (\operatorname{Id}_s J_r)*J_s = J_{r+s} $$