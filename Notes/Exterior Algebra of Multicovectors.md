...
**Def:** Given $\omega\in \bigwedge^k(V^*)$ and $\eta \in \bigwedge^l(V^*)$, we define their *wedge product* or *exterior product* to be the following $(k+k)$-covector: $$\omega \wedge \eta := \frac{(k+l)!}{k!l!}\text{Alt }(\omega\otimes \eta).$$

**Lemma:** Let $V$ be a $n$-dimensional $K$-vector space and let $(\varepsilon^1,\dots, \varepsilon^n)$ be a basis for $V^*$. For any multi-indices $I = (i_1,\dots, i_k)$ and $J = (j_1,\dots, j_l)$, $$\varepsilon^I \wedge \varepsilon^J = \varepsilon^{IJ}, $$where $IJ := (i_1,\dots, i_k,j_1,\dots, j_l)$ is obtained by concatenating $I$ and $J$. 
