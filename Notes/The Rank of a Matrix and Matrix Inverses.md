...
Links: [[Elementary Matrix Operations and Elementary Matrices]], [[Isomorphism of Vector Spaces]], [[Matrix Representation of Linear Transformations]], [[Determinants]], [[Null Space and Range]]

********Def:******** If $A \in {\cal{ M}}_{m \times n} (\Bbb F)$, we define the **rank** of $A$, denoted $r(A)$ or $\operatorname{rank}(A)$ to be the rank of the linear transformation $L_A: \Bbb{F^n \to F^n}$

**Def:** We say that the rank of an $m\times n$ matrix $A$ is *maximal* if $\text{rank}(A) = \min\{m, n\}$. 
...
********Th:******** The rank of any matrix equals the maximum number of its linearly independent columns; that is, the rank of a matrix is the dimension of th subspace generated by the columns
...
- $r(A) = r(A^\top)$
- The rank of a matrix equals the minimum number of its linearly independent rows; that is, the rank of a matrix is the dimension of the subspace generated by its rows
- The rows and columns of any matrix generate subspaces of the same dimension, numerically equal to the rank of a matrix
...
- $\text{rank}(UT) \le \text{rank}(T) , \text{rank}(U)$
- $\text{rank}(AB) \le \text{rank}(A), \text{rank}(B)$

**Lemma:** Let $A$ be an $m\times n$ matrix, and $k\in \Bbb N^+$. Then $\text{rank}(A) \ge k$ iff $A$ has a nonsingular $k\times k$ submatrix. Equivalently, $\text{rank}(A) \le k-1$ iff all $k\times k$ minors of $A$ vanish. 

**Prop:** For an integer $k\ge 0$, we define $D_r$ to be the subset of $\Bbb R^{m\times n}$ consisting of all $m \times n$ real matrices of rank at most $r$. Then $D_r$ is a closed subset of $\Bbb R^{m\times n}$. 

**Cor:** We define $D_\max$ to be the subset of $\Bbb R^{m\times n}$ consisting of all $m\times n$ matrices of maximal rank $r$. Then $D_\max$ is an open subset of $\Bbb R^{m\times n}$.  ^82654b
