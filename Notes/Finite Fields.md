...
**Prop:** If $a, b\in \Bbb Z$ such that $a$ and $b$ are not perfect squares, then $a$, $b$ or $ab$ is a square in $\Bbb F_p,$for every prime $p$. This happens because of the [[Legendre Symbols#Quadratic Reciprocity Law|law of quadratic reciprocity]]. 

**Cor:** Let $K = \Bbb Q(\theta) = \Bbb Q(\sqrt\alpha, \sqrt\beta)$ with $\alpha, \beta\in \Bbb Z$, is a [[Biquadratic Field Extensions|biquadratic extension]] and that $\theta = a + b\sqrt \alpha +c\sqrt \beta+d \sqrt{\alpha\beta}$, where $a, b,c, d\in \Bbb Z$ are integers. Then $\text{Irr}(\theta, \Bbb Q)$ has degree $4$, but is reducible modulo every prime $p$. This is impart because either $\alpha$, $\beta$ or $\alpha\beta$ is a square in $\Bbb F_p$. 
