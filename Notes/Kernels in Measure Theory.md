...
Links: [[Measures]], [[Measure Spaces and Measurable Spaces]], [[Lebesgue Integral on Measure Spaces]], [[Measurable Functions]], [[Finite Product of Measures]], [[Universally Measurable Sets]]
...
We can write the measure $\nu$ as $\mu K$ to make it clear the relationship with the kernel. 
...
**Prop:** Let $(X, {\scr A})$ and $(Y, {\scr B})$ be measurable spaces, and let $K$ be a kernel from $(X, {\scr A})$ to $(Y, {\scr B})$ such that $\sup\{K(x, Y) \mid x\in X\}$ is finite. For each $x\in X$ let $B \mapsto \overline K(x, B)$ be the restriction to $\scr B_*$ of the completion of the measure $B \mapsto K(x, B)$. Finally, for each finite measure $\mu$ on $(X, {\scr A})$ let $\mu K$ be the measure on $(Y, {\scr B})$ defined by $$(\mu K) (B) := \int K(x, B) \, \mu(dx).$$
- We see that $(x, B) \mapsto \overline K(x, B)$ is a kernel from $(X, {\scr A}_*)$ to $(Y, {\scr B}_*)$. 
- Suppose that $\mu$ is a finite measure on $(X, {\scr A})$ and $\overline \mu$ and $\overline{\mu K}$  are the restrictions to ${\scr A}_*$ and $\scr B_*$ of the completions of $\mu$ and $\mu K$. We see that $$\overline{\mu K} (B) = \int \overline K(x, B)\, \overline\mu(dx) $$holds for each $B\in \scr B_*$. 