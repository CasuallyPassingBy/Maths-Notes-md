...
Let $\{X_1, \dots X_n\}$ be a collection of metric spaces with metrics $d_1, \dots, d_n$, then we define the finite product of metric spaces $\prod_{k = 1}^n X_k$ with product metric $$d((x_1,\dots x_n), (y_1, \dots, y_n)) := \left(\sum_{k = 1}^n d_k(x_k, y_k)^2\right)^{1/2}.$$
We actually see that the topology generated by the metric topology is the same as the product topology of the metric spaces. 