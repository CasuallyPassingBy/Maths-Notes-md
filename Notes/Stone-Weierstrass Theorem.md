...
Let $K$ be a compact metric space and let $\cal A$ be a subalgebra of ${\cal C}(K)$ with unity that separates points. Then $\cal A$ is dense in ${\cal C}(K)$, i.e., for every continuous function $f:K \to \Bbb R$, there exists a sequence $(\phi_k)$ of functions in $\cal A$ that converges uniformly to $f$ in $K$
...
with $f_1, \dots f_n \in {\cal C}(X)$ and $g_1, \dots, g_n \in {\cal C}(K)$ and $n \in \Bbb N$
