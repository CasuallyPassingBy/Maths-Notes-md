...
$$ \operatorname{Var}[X] = \Bbb E[(X-\mu)^2] $$
...
- $\operatorname{Var}[X] \le \Bbb E[X^2]$
- $\operatorname{Var}[X+Y] = \operatorname{Var}[X] + \operatorname{Var}[Y] +2\Bbb E[(X-\Bbb E[X])(Y-\Bbb E[Y])]$, From this formula we can see that $\Bbb E[XY]=\Bbb E[X]\Bbb E[Y]$ is equivalent to ${\operatorname{Var}[X+Y] = \operatorname{Var}[X]+\operatorname{Var}[Y]}$.
