...
## Markov Inequality
Let $X \ge 0$, be a random variable with finite expected value. For any $\varepsilon > 0$, then $$\Bbb P(X \ge \varepsilon) \le \frac{\Bbb E[X]}{\varepsilon}$$
We can also look at the conditional version, let $\mathscr G$ be a sub-$\sigma$-algebra $$\Bbb P(X \ge \varepsilon \mid \mathscr G) \le \frac{E(X \mid \mathscr G)}{\varepsilon}$$We can also get different versions: $$\Bbb P(|X| \ge \varepsilon ) \le \frac{E|X|}{\varepsilon}$$$$\Bbb P(|X|^n \ge \varepsilon ) \le \frac{E|X|^n}{\varepsilon^n}$$

## Chebyshev Inquality
Let $X$ be a random variable with finite expected value $\mu$ and finite variance $\sigma$. For any $\varepsilon >0$, $$\Bbb P(|X - \mu |^2 \ge \varepsilon )  \le \frac{\sigma²}{\varepsilon^2}$$
