...
we can use induction and get that $$ \Bbb P(A_1 \cap \dots\cap A_n) = \Bbb P(A_1) \Bbb P(A_2\mid A_1) \Bbb P(A_3 \mid A_1 \cap A_2) \cdots \Bbb P(A_n \mid A_1 \cap \dots \cap A_{n-1}) $$
...
Let $B_1, B_2, \dots, B_n$ is a partition of $\Omega$ such that $\Bbb P(B_i) \ne 0$ for all $i$. Then for any event $A$ $$ \Bbb P(A) = \sum_{i = 1}^n \Bbb P(A \mid B_i ) \Bbb P(B). $$In particular, we have that if $\Bbb P(B) \ne 0, 1$, then
...
or equivalently$$ \Bbb P(A) = \Bbb P(A \mid B) $$Independent events and disjoint events are not the same, two events can be independent and not disjoint, and two events can be disjoint and not independent.
...
**Def:** We say that an infinite collection of events is independent if for any finite subcollection is independent. More explicitly, let $(\Omega, {\scr A}, \Bbb P)$ be a probability space, and let $\{A_i\}_{i \in I}$ be an indexed family of events. The events $A_i$, $i\in I$, are called *independent* if for each finite subset $I_0\in [I]^{<\omega}$ we have  $$\Bbb P\left(\bigcap_{i \in I_0} A_i \right) = \prod_{i\in I_0} \Bbb P(A_i).$$

**Prop:** Let $A_1, \dots, A_n$ are independent events then $$ \Bbb P(A_1 \cup \dots \cup A_n) = 1-\Bbb P(\Omega\setminus A_1)\Bbb P(\Omega\setminus A_2) \cdots \Bbb P(\Omega\setminus A_n) $$
