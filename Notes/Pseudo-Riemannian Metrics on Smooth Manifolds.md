...
**Def:** A *pseudo-Riemannian metric* on a smooth manifold $M$ is a smooth symmetric $2$-tensor field whose value is nondegenerate at each point, with the same signature everywhere on $M$. Pseudo-Riemannian metrics with signature $(n-1, 1)$, or $(1, n-1)$ depending on the convention used, are called *Lorentz metrics*.
